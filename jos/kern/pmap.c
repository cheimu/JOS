...
mem_init_mp(void)
{
	...
	//uint32_t k_start = KSTACKTOP - KSTKSIZE;
...
page_init(void)
{
	...
	for (int i = 1; i < PGNUM(IOPHYSMEM); i++) {
		struct e820_entry* ent = e820_map.entries;
		int is_inuse = 1;
		...
		if (create == false) {
			...
			if (newPTP == NULL) {
				...
				*pdeaddr = page2pa(newPTP) | PTE_W | PTE_P | PTE_U;
				pte_t* res_addr = (pte_t*) page2kva(newPTP);
				return (pte_t*) (&res_addr[PTX(va)]);
			}
		}
	// exsit
	} else {
		// find ppn, shift to get rid of flags and add page table offset to get pte address
		pte_t* res_addr = KADDR(PTE_ADDR(*pdeaddr));
		pte_t* pteaddr = &res_addr[PTX(va)];
		//(pte_t*) (KADDR(*pdeaddr & 0xfffff000) + PTX(va));
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (int i = 0; i < size / PGSIZE; i++) {
		uintptr_t va_real = va + i * PGSIZE;
		physaddr_t pa_real = pa + i * PGSIZE;

		// page table walk to find page table entry
		
		pte_t* pgt_entry = pgdir_walk(pgdir, (const void*) va_real, 1);
		*(pgt_entry) = pa_real | perm | PTE_P;

		/**(pgdir + PDX(va)) =  pgt_dir_entry | PTE_P;
		*/
	}
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	// check if user provides a valid pointer
	if (pte_store != 0) {
		*pte_store = pte_entry;
	}
	if (!(*pte_entry & PTE_P))
		return NULL;
	physaddr_t pa = PTE_ADDR(*pte_entry) | PGOFF(va);
...
check_kern_pgdir(void)
{
	...
	// check kernel stack
	// probably will cause bugs somehow
