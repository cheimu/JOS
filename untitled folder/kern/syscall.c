	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// check if srcva is read-only in srcenvid's address space.
...
static void
sys_set_console_color(int color) {
	console_color = color;
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_blk_write:
		return sys_blk_write(a1, (void*) a2, (int) a3);
	case SYS_blk_read:
		return sys_blk_read(a1, (void*) a2, (int) a3);
